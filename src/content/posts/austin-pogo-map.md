...
wanted to do some of it together, but it is late July in Austin and Sunday's temperature is
...
Ideally two gyms in range of the table so we could just cycle through raids while we munched.

## The 80 meter restriction

In PoGo you can interact with a gym from ~80 meters away, so a restaurant only
counts if the gym is basically in the building. Two gyms from one seat is much
harder. They have to be within 160 meters of each other and you have to be
sitting between them.

## No shortcut on the data

I tried to query gym locations and score every breakfast spot at once,
...
crowd-sourced and there is no way to pull from it
systematically.
...
4. Log the gym's location (alongside other metadata)

For the restaurants, I grabbed the coordinates from the Google Maps URL and
used OpenStreetMap to fill in details about the place.
...
- **Starting from food**: I picked places I already thought had at least decent
  ...
  restaurant they sit at i.e. "Kerbey Lane Cafe" or "Summer Moon Coffee."
...
I put everything I logged into a small JSON file and had Claude build me something to read it. The plan is to keep adding
